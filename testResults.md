...
The following table summarizes the results produced by Rubi, Mathematica and Maple on the integration test-suite.  The first column gives the total number of problems for the integrand type specified in the last column.  The column for each integrator provides the following information:
